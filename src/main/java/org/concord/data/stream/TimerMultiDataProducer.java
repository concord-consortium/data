 ...
 *
 */
public abstract class TimerMultiDataProducer extends DefaultMultipleDataProducer
	implements ActionListener
{
	...
	{
		values = new float[numberValues];
		getDataDescription().setChannelsPerSample(numberValues);
	}
	...
	{		
		updateValues(currentTime, values);
		addValues(values);
		currentTime += getDataDescription().getDt();
	}
    
	protected abstract void updateValues(float t, float [] values);
